...
def Insert():
    ...
    if(id=="" or name=="" or phno==""):
        ...
        con=mysql.connect(host="localhost",user="root",password="YOUR_PASSWORD",database="student")
        con=sqlite3.connect("studentdb.db")
        cur=con.cursor()
        cur.execute("create table student(id int,name varchar(20),phno int(11))")
        cur.execute("insert into student values('"+id+"','"+name+"','"+phno+"')")
        cur.execute("Commit")
